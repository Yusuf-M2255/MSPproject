...
void PacPlayMode(int x)
{
	...
	{
		for (int ir = 0; ir < fullPix.size(); ir++)
		{
			FloatRect nextpos = ghostr.getGlobalBounds();
			nextpos.left += ghostSpeed.x * deltaTime;
			nextpos.top += ghostSpeed.y * deltaTime;

			if (fullPix[ir].getGlobalBounds().intersects(nextpos))
			{
				// Right
				if (ghostr.getGlobalBounds().left < fullPix[ir].getGlobalBounds().left
					&& ghostr.getGlobalBounds().left + ghostr.getGlobalBounds().width < fullPix[ir].getGlobalBounds().left + fullPix[ir].getGlobalBounds().width
					&& ghostr.getGlobalBounds().top < fullPix[ir].getGlobalBounds().top + fullPix[ir].getGlobalBounds().height
					&& ghostr.getGlobalBounds().top + ghostr.getGlobalBounds().height > fullPix[ir].getGlobalBounds().top
					)
				{
					ghostSpeed.x = 0.f;
					ghostr.setPosition(fullPix[ir].getGlobalBounds().left - ghostr.getGlobalBounds().width, ghostr.getGlobalBounds().top);
				}
				// Left
				else if (ghostr.getGlobalBounds().left > fullPix[ir].getGlobalBounds().left
					&& ghostr.getGlobalBounds().left + ghostr.getGlobalBounds().width > fullPix[ir].getGlobalBounds().left + fullPix[ir].getGlobalBounds().width
					&& ghostr.getGlobalBounds().top < fullPix[ir].getGlobalBounds().top + fullPix[ir].getGlobalBounds().height
					&& ghostr.getGlobalBounds().top + ghostr.getGlobalBounds().height > fullPix[ir].getGlobalBounds().top
					)
				{
					ghostSpeed.x = 0.f;
					ghostr.setPosition(fullPix[ir].getGlobalBounds().left + fullPix[ir].getGlobalBounds().width, ghostr.getGlobalBounds().top);
				}
				// Top
				else if (ghostr.getGlobalBounds().top > fullPix[ir].getGlobalBounds().top
					&& ghostr.getGlobalBounds().top + ghostr.getGlobalBounds().height > fullPix[ir].getGlobalBounds().top + fullPix[ir].getGlobalBounds().height
					&& ghostr.getGlobalBounds().left < fullPix[ir].getGlobalBounds().left + fullPix[ir].getGlobalBounds().width
					&& ghostr.getGlobalBounds().left + ghostr.getGlobalBounds().width > fullPix[ir].getGlobalBounds().left
					)
				{
					ghostSpeed.y = 0.f;
					ghostr.setPosition(ghostr.getGlobalBounds().left, fullPix[ir].getGlobalBounds().top + fullPix[ir].getGlobalBounds().height);
				}
				//  Bottom
				else if (ghostr.getGlobalBounds().top < fullPix[ir].getGlobalBounds().top
					&& ghostr.getGlobalBounds().top + ghostr.getGlobalBounds().height < fullPix[ir].getGlobalBounds().top + fullPix[ir].getGlobalBounds().height
				...
				{
					ghostSpeed.y = 0.f;
					ghostr.setPosition(ghostr.getGlobalBounds().left, fullPix[ir].getGlobalBounds().top - ghostr.getGlobalBounds().height);
				}
...
int main()
{
	...
	while (window.isOpen())
	{
		...
		Vector2f pacmanSpeed = { 200, 200 };
		Vector2f ghostSpeed = { 100, 100 };
		...
		for (int y = 0; y < 29; ++y) {
			for (int x = 0; x < 31; ++x) {
				...
				else if (mapSketch[x][y] == ' ' && x % 2 == 0 && y % 2 == 0 && mapSketch[x][y] != '0')
				{
		...
		else if (Keyboard::isKeyPressed(Keyboard::Num4))
		{
			GameMode = 4;
			ghostr.setPosition(935, 450);
			ghostp.setPosition(935, 510);
			ghostc.setPosition(1005, 510);
			ghosto.setPosition(865, 510);
		}
		if (GameMode != 0)
